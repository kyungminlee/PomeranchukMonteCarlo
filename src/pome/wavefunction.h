...
struct CflCache
{
  ...
  //Complex log_determinant;
  //Complex log_determinant_old; // with theta_old
...
public:
  ...
  explicit CflWavefunction(Geometry const & geometry,
                           Integer n_electron,
    ...
    , maximum_displacement_difference_(4.0)
    ...
    , maximum_displacement_kappa_difference_(4.0)
  ...
  void init_displacements(Complex const * displacements)
  {
    ...
    for (Integer i = 0; i < ne; ++i) {
      ...
      Complex kvec(dvec.imag() * 2.0, -dvec.real() * 2); //!< d=ik/2,  k=-2id
...

    {
      ...
      displacement_kappa_mean_ = displacement_total_ * inv_ne;
      displacement_inverse_kappa_mean_ = displacement_total_ * inv_ne;
    }
                         ...
                         RandGen& gen) const
  {
    ...
    auto angle = distribution_01_(gen) * 2.0 * M_PI;
                      ...
                      CflCache & cache) const
  {
    ...
    //auto const & ikd = displacement_inverse_kappas_;
    ...
    for (Integer k = 0; k < nel; ++k) {
      if (k != i) {
        ...
        auto phi = geometry_.theta1(w) / geometry_.theta1_prime();
                       ...
                       CflCache& cache) const
  {
    ...
    auto z_mean = z_total * inv_nel;
    auto y_total = z_total.imag();

    auto xp = std::exp((y_total * y_total * inv_nel
                        - y_squared_total
                        + momentum_total_.imag() * y_total * inv_nel)
                       * 0.5 * inv_nel);
    ...
    for (Integer i = 0; i < nel; ++i) {
      for (Integer j = 0; j < nel; ++j) {
        ...
        for (Integer k = 0; k < nel; ++k) {
          if (k != i) {
            factor *= cache.theta(i, k, j);
          } // if (k != i)
        } // for k
        cache.slater(i, j)
          = factor * xp * std::exp(zs(i) * displacement_inverse_kappas_(j));
      } // for j
    } // for i
    cache.log_determinant_abs_squared 
      = 2.0 * kore::array::linalg::log_determinant_abs(cache.slater);
  }
  //! \name Getter
  //@{
